...
const restartButton = document.getElementById("restart_el");
const pressedItem = document.getElementById("pressed_item");

restartButton.addEventListener('click', () => {
   pressedItem.style.opacity = "0.75"
   location.reload()

  setTimeout(() => {
    pressedItem.style.opacity = "1"
  }, 100)
})
...
function showGrowthDiv() {
    ...
    document.querySelector("#report-section").classList.add("animate__bounceInRight")
    document.querySelector('#report-section').style.height = 'inherit'
    document.querySelector('#yearBreakdow').style.opacity = 1
    document.querySelector("#yearBreakdow").classList.add("animate__bounceInRight")
    document.querySelector("#input_element").classList.add("animate__backInUp")
    document.querySelector('#yearBreakdow').style.height = 'inherit'
}

// remove previous values
function removePreviousNumbers() {
  document.querySelectorAll("#breakdow p").forEach((elem) => {
    elem.remove();
  });
}
...
function buildValues(
  ...
  document.querySelector(
    "#totalValue"
  ).innerHTML = `Total Value after ${duration} years: <span>${balanceEnd}</span>`;
}
